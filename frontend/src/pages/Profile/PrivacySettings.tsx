...
export function PrivacySettings() {
  ...
  const updatePrivacyLocal = useUserStore((state) => state.updatePrivacyLocal);
            ...
            {fields.map((field) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(event) => {
                    updatePrivacyLocal({ [field.key]: event.target.checked });
                    showToast("Đã cập nhật public view trong local state.");
                  }}
                  type="checkbox"
                />
              </label>
        ...
        <Card padding="lg">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-mutedForeground">Không tạo token thật, chỉ mô phỏng hiển thị.</p>
            </div>
            <Badge tone="pending">Mock</Badge>
          </div>
            ...
            <div className="space-y-3">
              {settings.showBloodType ? (
                <div className="rounded-card bg-infoBg p-4">
                  ...
                  <p className="text-lg font-semibold text-blue-900">{profile.bloodType}</p>
                </div>
              ) : null}
              {settings.showAllergies ? (
                <div className="rounded-card bg-dangerBg p-4">
                  <p className="text-xs text-red-700">Dị ứng</p>
                  <p className="font-semibold text-red-900">{profile.allergies}</p>
                </div>
              ) : null}
              {settings.showEmergencyContact ? (
                <div className="rounded-card bg-successBg p-4">
                  ...
                  <p className="font-semibold text-green-900">{profile.emergencyContact}</p>
                </div>
              ) : null}
            </div>
          </div>
