...
* `GET /users/me/export`: Tải về dữ liệu (JSON/PDF). Hỗ trợ `?format=json|pdf&scope=profile,diaries,medical_records,prescriptions`.
...
* `GET /doctors/search-patients`: Bác sĩ tìm kiếm bệnh nhân (theo tên, SĐT). Chỉ trả về thông tin public của bệnh nhân.
