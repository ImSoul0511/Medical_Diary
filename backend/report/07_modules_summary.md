...
* **Nhiệm vụ:**
  ...
  * **Email nhắc nhở uống thuốc (`/prescriptions/internal/send-reminders`):** API nội bộ quét cơ sở dữ liệu các cữ uống sắp tới để tự động gửi email thông báo nhắc nhở qua Resend API (kích hoạt bằng Railway Cron Service thông qua token an toàn `X-Internal-Token`).
