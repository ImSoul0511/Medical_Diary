...
- Backend hiện là FastAPI, frontend gọi API qua lớp wrapper dưới `src/api/`. "NodeJS" ở frontend nên hiểu là runtime/tooling cho React/Vite, không thay thế backend hiện tại.
...
- Mọi task triển khai có liên quan đến Supabase SDK ở frontend hoặc thay đổi cách gọi Supabase trực tiếp đều phải confirm lại với người dùng trước khi thực hiện. Mặc định frontend gọi backend API qua `src/api/`; không tự ý thêm Supabase SDK, Supabase client, Realtime subscription, Storage upload trực tiếp từ frontend nếu chưa được xác nhận.
...
- Access token chỉ được lưu trong RAM auth store.
- Refresh token do backend quản lý bằng HttpOnly cookie; frontend JavaScript không được đọc hoặc persist token vào localStorage/sessionStorage.
...
`src/api/apiClient.ts` cần:

- Đọc `VITE_API_BASE_URL`.
- Là nơi duy nhất được import `axios` và gọi `axios.create`.
- Tự gắn `Authorization: Bearer <token>` từ RAM auth store nếu có.
- Set `Content-Type: application/json` cho JSON request.
- Không set `Content-Type` khi gửi `FormData` cho `register-doctor`.
- Không bật `withCredentials` global; chỉ bật ở login/refresh/logout trong auth API wrapper.
...
- Nếu `401`: dùng single-flight refresh queue; chỉ request 401 đầu tiên được gọi `/auth/refresh`, các request sau đợi token mới.
- Sau refresh thành công: gọi `authStore.setAccessToken(newToken)` trước khi retry request và giải phóng queue.
- Nếu refresh thất bại: reject toàn bộ queue, logout, clear RAM auth state và redirect login.
- `/auth/refresh` và `/auth/logout` phải đánh dấu `skipAuthRefresh` để không tự retry chính nó.
- Nếu `401` vẫn xảy ra sau một lần `_retry`: clear authStore và redirect login.
...
6. Lưu access token vào RAM `authStore`, gọi `GET /users/me`.
...
- Nếu endpoint admin chưa sẵn sàng, UI phải hiển thị trạng thái chưa hỗ trợ rõ ràng; không fallback sang dữ liệu mock khi API lỗi.
...
5. Tạo/cập nhật `src/api/apiClient.ts` và các wrapper endpoint trong `src/api/*Api.ts`.
...
11. Implement Doctor/Admin pages ở trạng thái ready-for-API; không dùng mock data làm fallback khi API lỗi.
