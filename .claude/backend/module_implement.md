...
6. **Khi triển khai tính năng liên quan đến gửi email:** Bắt buộc phải đọc kỹ tài liệu hướng dẫn tại `.claude/backend/EMAIL.md` để sử dụng đúng email helper và dùng phương pháp chạy ngầm (BackgroundTasks hoặc asyncio.to_thread) để tránh làm nghẽn Event Loop.
