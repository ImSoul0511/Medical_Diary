...
* `GET /doctors/search-patients`: Bác sĩ tìm kiếm bệnh nhân. Chỉ trả về thông tin cơ bản.
...
* **Medical Records (Bác sĩ ghi nhận):**
  ...
  * **Personal Medical Documents (Tài liệu cá nhân tự tải):**
    * `POST /medical-records/documents/upload`: Bệnh nhân tự tải lên tài liệu cá nhân của mình.
    * `GET /medical-records/documents/mine`: Bệnh nhân xem danh sách tài liệu cá nhân của chính mình.
    * `GET /medical-records/documents/patient/{patient_id}`: Bác sĩ xem danh sách tài liệu cá nhân của bệnh nhân. **Yêu cầu:** `hasRole("doctor")` + consent scope `patient_documents`.
    * `DELETE /medical-records/documents/{document_id}`: Bệnh nhân xóa tài liệu cá nhân đã tải lên.
* **Prescriptions:**
...
* `POST /notifications/read-all`: Đánh dấu tất cả thông báo của User hiện tại là đã đọc.
